...
import pool6 from "../database6.js";
...
passport.use('local.signupqui', new LocalStrategy({
    usernameField: 'usuario',
    passwordField: 'password',
    passReqToCallback: 'true'
}, async (req, usuario, password, done) => {
  ...
  if (nivel == undefined){
    ...
    try {  
        ...
        const verif  = await pool6.query('select * from usuarios where usuario = ?',[usuario])
        if (verif.length>0){
            req.flash('message', 'error, usuario existente')
        }else{
        newUser.password = await helpers.encryptPassword(password)
        try {
            const result = await pool6.query('INSERT INTO usuarios  set password=?, usuario=?,nombre=?,tel=?,mail=?,nivel=?', [newUser.password, usuario,nombre,tel, mail,nivel])
...
passport.use('local.signinqui', new LocalStrategy({
    usernameField: 'usuario', // usuario es el nombre que recibe del hbs
    passwordField: 'password',
    passReqToCallback: 'true' // para recibir mas datos 

}, async (req, usuario, password, done) => {  // que es lo que va a hacer 
    const rows = await pool6.query('SELECT * FROM usuarios WHERE usuario = ?', [usuario])
    if (rows.length > 0) {
        const usuario = rows[0]

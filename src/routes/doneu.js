...
router.get("/traerlotes", async (req, res) => {
    ...
    try {
        const lot = await pool.query('select * from lotes left join (select id_lote,id_cliente from ventas) as sel on lotes.id=sel.id_lote  join (select id as idp, nombre from clientes) as sel2 on sel.id_cliente=sel2.idp')

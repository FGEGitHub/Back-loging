    ...
    try {
        ...
        if (estados[numero] === 'esperando_dni') {
            ...
            const resultado = await pool.query(
                'SELECT * FROM inscripciones_fiscales WHERE dni = ? AND edicion = 2025',
                [dni]
            );

            if (resultado.length > 0) {
                const fiscal = resultado[0];
                const fecha = fiscal.fecha_carga;
                ...
                if (fiscal.estado === 'asignado') {
                    ...
                    await message.reply(`📝 Estás inscripto pero todavía no fuiste contactado.\nTe registraste el día ${fecha}. Pronto nos contactaremos.`);
                }
            } else {
...
ORDER BY apellido;
    ...
    const obrasChicos = await pool.query(`
      ...
      FROM marketing.dtc_chicos
      WHERE psico IS NULL 
        AND obra_social_cual IS NOT NULL 
    ...
    res.json(toJSONSafe([chiques, estadisticas, detalleObras]));
...
router.post("/modificarusuario", async (req, res) => {
  ...
  try {
    ...
    if (!obra_social) obra_social = "Sin determinar";
...
router.post("/nuevaclasetaller", async (req, res) => {
  ...
  console.log(id_tallerista, fecha, titulo, detalle)
  ...
  try {
    ...
    await pool.query('insert dtc_clases_taller  set id_tallerista=?, fecha=?,titulo=?, descripcion=?', [id_tallerista, fecha, titulo, detalle])
...
router.post("/borrarcolacion", async (req, res) => {
  const { id } = req.body;

  try {
    // Obtener la ubicación del archivo desde la base de datos
    const result = await pool.query('SELECT ubicacion FROM dtc_colacion WHERE id = ?', [id]);
    ...
    if (result.length > 0) {
      ...
      // Eliminar la entrada de la base de datos
      await pool.query('DELETE FROM dtc_colacion WHERE id = ?', [id]);
...
router.get('/traeretapacocinacadia/', async (req, res) => {
  const existe = await pool.query('SELECT * FROM dtc_etapa_cadia  ORDER BY proyecto ASC, etapa DESC');

...
/*   let {
  ...
  try {
    ...
    const yainsc = await pool.query(
      'SELECT * FROM inscripciones_carnaval WHERE id_persona = ? AND id > 590 AND detalle = ?',
      [pers[0]['id'], "Capibara-Semiper-Costura"]
    );
    ...
    if (yainsc.length > 0) {
      ...
      await pool.query(
        `INSERT INTO inscripciones_carnaval 
         ...
         VALUES (?, ?, ?, ?, ?,?,?,?,?,?,?)`,
        [ fecha, dni, pers[0]['id'], "Capibara-Semiper-Costura", curso,profesion,alumna_anterior,tiene_espacio,enseniar,curso_adic,agregar_whatsap]
      );

...
router.get('/clases/:id', async (req, res) => {
    ...
    turnos = await pool.query('select *, id as turnoid  from turnos join (select  id as idcurso, nombre from cursos  )as cursoss on turnos.id_curso=cursoss.idcurso where etapa=5 and turnos.id_coordinador =? ', [id])
    ...
    for (ii in turnos) {
      ...
      tot = await pool.query('select * from cursado  join (select id as idi, estado from inscripciones) as sel on cursado.id_inscripcion=sel.idi where id_turno= ? and estado="Asignada a curso"',[turnos[ii]['id']])
     nuev = {
      id: turnos[ii]['id'],

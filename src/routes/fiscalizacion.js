...
router.get('/todaslasasignacionesparaasistencia/:id', async (req, res,) => {
    ...
    try {
        ...
        cant = await pool.query('select * from mesas_fiscales join (select id_usuario, id as idescuela from escuelas) as sele on mesas_fiscales.id_escuela=sele.idescuela left join (select mesa as mesaa , fiscaliza, edicion, dni from asignaciones_fiscales where edicion=2025) as sel on mesas_fiscales.id=sel.mesaa join (select dni as dnip,nombre,apellido from personas_fiscalizacion) as sel2 on sel.dni=sel2.dnip where id_usuario=? ', [id])
        ...
        res.json([estr, cant, supl.length, esc.length,esccuelanombre])
    } catch (error) {
        console.log(error)
        res.send('algo salio mal')
    }
...
router.post("/enviarcalificacion", async (req, res) => {
    const { id, valor } = req.body;
    try {
        await pool.query(
            'UPDATE asignaciones_fiscales SET calificacion=? WHERE id=?',
            [valor, id]
        );

        res.json('calificación guardada');
    } catch (error) {
        console.log(error);
        res.json('error');
    }
